  ...
  /**
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "PEVRT::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "PEVRT::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "PEVRT::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "PEVRT::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying PEVRT (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "PEVRT::_writeCheckpoint: block number exceeds 32 bits");
...
// This contract handles powering and uncharging of your pEVRT, Everest DAO's staking token.
contract EVRTCharger is ERC20WithVoting("Charged EVRT", "pEVRT"), Ownable {
    using SafeERC20 for IERC20;

    IERC20 public immutable evrt;
    ...
    // Tracks total EVRT that has been collected via the PHP.
    ...
    uint256 public rollingStartEvrtBalance;
    ...
    event DailyRewardsReceived(uint256 indexed timestamp, uint256 amountEvrt);
    ...
    // address.
    constructor(IERC20 _evrt, uint256 _paperHandsPenalty) {
        evrt = _evrt;
    ...
    //returns current exchange rate of pEVRT to EVRT, scaled up by 1e18
    function currentExchangeRate() public view returns(uint256) {
        uint256 totalShares = totalSupply();
        if(totalShares == 0) {
            return 1e18;
        }
        return (evrtBalance() * 1e18) / totalShares;
    }

    //returns user profits in EVRT (returns zero in the case that user has losses due to previous PHP)
    function userProfits(address stakingAddress) public view returns(uint256) {
        uint256 userDeposits = deposits[stakingAddress];
        uint256 userWithdrawals = withdrawals[stakingAddress];
        uint256 totalShares = totalSupply();
        uint256 shareValue = (balanceOf(stakingAddress) * evrtBalance()) / totalShares;
    ...
    function evrtBalance() public view returns(uint256) {
        return evrt.balanceOf(address(this));
    }

    //tracks the amount of EVRT the nest has received over the last 24 hours
    function rewardsReceived() public view returns(uint256) {
        // Gets the current EVRT balance of the nest
        uint256 totalEvrt = evrtBalance();
        ...
        // net rewards received is (new evrt  balance - old evrt balance) + (withdrawals - deposits)
        return ((totalEvrt + withdrawalsDuringPeriod) - (depositsDuringPeriod + rollingStartEvrtBalance));
    }
    ...
    // Enter the nest. Pay some EVRT. Earn some shares.
    // Locks EVRT and mints pEVRT
    function enter(uint256 _amount) external {
        // Gets the amount of EVRT locked in the contract
        uint256 totalEvrt = evrtBalance();
        // Gets the amount of pEVRT in existence
        uint256 totalShares = totalSupply();
        // If no pEVRT exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || totalEvrt == 0) {
            _mint(msg.sender, _amount);
        }
        // Calculate and mint the amount of pEVRT the EVRT is worth.
        // The ratio will change overtime, as pEVRT is burned/minted and EVRT
        // deposited + gained from fees / withdrawn.
        else {
            uint256 what = (_amount * totalShares) / totalEvrt;
            _mint(msg.sender, what);
        }
        //track deposited EVRT
        deposits[msg.sender] = deposits[msg.sender] + _amount;
        totalDeposits += _amount;
        // Lock the EVRT in the contract
        evrt.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    // Leave the vault. Claim back your EVRT.
    // Unlocks the staked + gained EVRT and redistributes pEVRT.
    function leave(uint256 _share) external {
        // Gets the amount of pEVRT in existence
        uint256 totalShares = totalSupply();
        // Gets the EVRT balance of the nest
        uint256 totalEvrt = evrtBalance();  
        // Calculates the amount of EVRT the pEVRT is worth
        uint256 what = (_share * totalEvrt) / totalShares;
        //burn pEVRT
        _burn(msg.sender, _share);
        //send withdrawn EVRT
        uint256 tax = (what * paperHandsPenalty) / MAX_BIPS;
        uint256 toSend = what - tax;
        // Send the person's EVRT to their address
        evrt.safeTransfer(msg.sender, toSend);
        //track withdrawn EVRT
    ...
    function _transfer(
        ...
        // Gets the EVRT balance of the nest
        uint256 totalEvrt = evrtBalance();
        uint256 evrtValueOfShares = (amount * totalEvrt) / totalShares;
        // take part of profit tracking
        deposits[recipient] += evrtValueOfShares;
        withdrawals[sender] += evrtValueOfShares;
